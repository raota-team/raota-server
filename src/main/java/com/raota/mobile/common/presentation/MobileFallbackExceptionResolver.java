 ...
 *
 * <p>
 * advice는 선택한 예외를 조건부로 거절할 수 없어 전역 advice로 처리하면 v1 응답까지 변경된다. 따라서 먼저 실행되고 거절할 수 있는
 * resolver를 사용한다. 컨트롤러 메서드가 결정된 경우에는 {@link MobileExceptionAdvice}가 처리하며, 그 밖의 요청 중 지정된
 * v2 오류만 이 resolver가 처리한다.
 * </p>
 */
@Component
    ...
    public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
        ...
        try {
            ...
            objectMapper.writeValue(response.getWriter(),
                    MobileApiResponse.failure(MobileError.of(failure.code(), failure.message()))
