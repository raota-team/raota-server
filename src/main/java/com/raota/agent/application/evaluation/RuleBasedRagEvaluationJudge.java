...
public class RuleBasedRagEvaluationJudge implements RagEvaluationJudge {
    ...
    @Override
    public RagEvaluationJudgeResult suggest(
            RagEvaluationCase evaluationCase,
            RagExecutionResult executionResult
    ) {
        ...
        evaluationCase.forbiddenClaims().stream()
        ...
        String verdict = groundedness >= 1 && unsupportedClaims.isEmpty() && sufficiencyCorrect
                ? "PASS" : "REVIEW";
        String reason = unsupportedClaims.isEmpty()
                ? "필수 사실·금지 주장 규칙을 결정적으로 검사한 제안입니다."
