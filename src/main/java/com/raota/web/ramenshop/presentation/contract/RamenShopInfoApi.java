...
import com.raota.web.ramenshop.presentation.response.RamenShopMenuOptionsResponse;
import com.raota.web.ramenshop.presentation.response.RamenShopResponse;
import com.raota.web.ramenshop.presentation.request.RamenShopSearchRequest;
import com.raota.web.ramenshop.presentation.response.RamenShopBasicInfoResponse;
...
import com.raota.web.ramenshop.presentation.response.RecentVerifiedShopResponse;
...
public interface RamenShopInfoApi {
    ...
    ResponseEntity<ApiResponse<RamenShopBasicInfoResponse>> getShopDetailInfo(
            @Parameter(description = "가게 ID", required = true) Long shopId, Long memberId);
    ...
    ResponseEntity<ApiResponse<PageResponse<RamenShopResponse>>> getShopList(
            ...
            @Parameter(description = "페이지 크기", example = "12") @RequestParam(defaultValue = "12") int size,
            RamenShopSearchRequest request);

    @Operation(summary = "가게 북마크 토글", description = "가게를 찜하거나 해제합니다.")
    @ApiResponses({ @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공") })
    ResponseEntity<ApiResponse<Boolean>> toggleBookmark(@Parameter(description = "가게 ID", required = true) Long shopId,
            @Parameter(hidden = true) Long memberId);

    @Operation(summary = "가게 정보 제보하기", description = "가게의 영업시간 오류, 폐업 등을 제보합니다.")
    @ApiResponses({ @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "성공") })
    ResponseEntity<ApiResponse<Void>> reportShop(@Parameter(description = "가게 ID", required = true) Long shopId,
            @Parameter(hidden = true) Long memberId, RamenShopReportRequest request);
