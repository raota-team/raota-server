...
import com.raota.web.ramenshop.domain.model.Bookmark;
...
public class BookmarkService {
    ...
    public boolean toggleBookmark(Long memberId, Long shopId) {
        boolean bookmarked = bookmarkRepository.findByMemberProfileIdAndRamenShopId(memberId, shopId).map(bookmark -> {
            ...
            Bookmark bookmark = Bookmark.builder().memberProfile(member).ramenShop(shop).isDeleted(false).build();
