...
import com.raota.web.ramenlog.presentation.api.response.ProofPictureInfoResponse;
import com.raota.web.ramenlog.presentation.api.response.RamenShopProofPictureResponse;
...
public interface RamenLogRepository extends JpaRepository<RamenLog, Long>, JpaSpecificationExecutor<RamenLog> {
    ...
    @EntityGraph(attributePaths = { "author", "ramenShop" })
    Page<RamenLog> findAll(org.springframework.data.jpa.domain.Specification<RamenLog> specification,
            Pageable pageable);
    ...
    @Query(value = """
            select new com.raota.web.ramenlog.presentation.api.response.ProofPictureInfoResponse(
                p.id,
                true,
                p.imageUrl
            )
    ...
    @Query(value = """
            select new com.raota.web.ramenlog.presentation.api.response.RamenShopProofPictureResponse(
                p.id,
                p.author.id,
                p.imageUrl,
                p.author.nickname,
    ...
    @Query("""
            select new com.raota.web.ramenlog.domain.repository.RecentVerifiedShopProjection(
                s.id,
                s.name,
