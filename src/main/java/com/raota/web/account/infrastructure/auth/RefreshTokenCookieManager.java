...
/**
 * Refresh Token을 브라우저 쿠키에 안전하게 보관하고 관리하는 매니저. HttpOnly, Secure 설정 등을 통해 토큰 탈취를 방지한다.
 */
@Slf4j
@Component
public class RefreshTokenCookieManager {
    ...
    /**
     * 새로운 Refresh Token을 담은 보안 쿠키를 생성한다. - HttpOnly: 자바스크립트에서 접근 불가 (XSS 방지) - Secure:
     * HTTPS 연결에서만 전송 - SameSite: CSRF 공격 방지 설정
     */
    public ResponseCookie createRefreshTokenCookie(String refreshToken) {
        ...
        ResponseCookie.ResponseCookieBuilder builder = ResponseCookie
            .from(authProperties.cookie().refreshTokenName(), refreshToken)
            .httpOnly(true)
            .secure(secure)
    ...
    public ResponseCookie clearRefreshTokenCookie() {
        ResponseCookie.ResponseCookieBuilder builder = ResponseCookie
            .from(authProperties.cookie().refreshTokenName(), "")
            .httpOnly(true)
