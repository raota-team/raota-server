...
public class RestAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
        ...
        String message = expiredToken && RestSecurityErrorWriter.isV2Request(request) ? EXPIRED_ACCESS_TOKEN_MESSAGE
                : responseMessage(authException);
        errorWriter.write(request, response, HttpServletResponse.SC_UNAUTHORIZED, v2Code, message);
    }

    private String responseMessage(AuthenticationException authException) {
        if (authException instanceof JwtAuthenticationException && authException.getMessage() != null
