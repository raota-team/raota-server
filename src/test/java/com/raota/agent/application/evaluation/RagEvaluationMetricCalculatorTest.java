...
class RagEvaluationMetricCalculatorTest {
    ...
    void calculatesMobileTopOneAndDiagnosticTopSixMetrics() {
        Map<String, Double> metrics = RagEvaluationMetricCalculator.calculateSearch(List.of(3L, 2L, 99L),
                List.of(new RagExpectedShop(2L, 3), new RagExpectedShop(3L, 1)), false, 1, 6);
    ...
    void calculatesEmptyResultAccuracyWithoutGoldItems() {
        assertThat(RagEvaluationMetricCalculator.calculateSearch(List.of(), List.of(), true, 1, 6))
            .containsEntry("emptyResultAccuracy", 1.0);

        assertThat(RagEvaluationMetricCalculator.calculateSearch(List.of(1L), List.of(), true, 1, 6))
            .containsEntry("emptyResultAccuracy", 0.0);
    }

    @Test
    void averagesEachMetricAcrossCases() {
        Map<String, Double> average = RagEvaluationMetricCalculator
            .average(List.of(Map.of("hitrateAt1", 1.0, "recallAt6", 0.0), Map.of("hitrateAt1", 0.0, "recallAt6", 1.0)));

        assertThat(average).containsEntry("hitrateAt1", 0.5).containsEntry("recallAt6", 0.5);
    }
    ...
    void calculatesGenerationCoverageAndFallbackAccuracy() throws Exception {
        RagEvaluationCase evaluationCase = new RagEvaluationCase("summary-1", RagEvaluationCaseType.SUMMARY,
                RagEvaluationSplit.DEV, JsonMapper.builder().build().createObjectNode().put("shopId", 1),
                "AUTHENTICATED", List.of(), false, List.of("시오라멘", "웨이팅"), List.of(), List.of("확정할 수 없는 가격"), false,
                false, 1, 6);
        ...
        Map<String, Double> metrics = RagEvaluationMetricCalculator.calculateGeneration(evaluationCase, response,
                false);
    ...
    void rejectsGenerationResponseWithWrongShape() throws Exception {
        RagEvaluationCase evaluationCase = new RagEvaluationCase("chat-1", RagEvaluationCaseType.CHAT,
                RagEvaluationSplit.DEV, JsonMapper.builder().build().createObjectNode(), "AUTHENTICATED", List.of(),
                false, List.of(), List.of(), List.of(), false, false, 1, 6);

        Map<String, Double> metrics = RagEvaluationMetricCalculator.calculateGeneration(evaluationCase,
                JsonMapper.builder().build().readTree("{\"body\":\"답변\"}"), false);
