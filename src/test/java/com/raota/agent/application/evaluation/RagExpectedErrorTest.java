...
class RagExpectedErrorTest {
    ...
    void classifiesMatchingValidationErrorWithoutCallingItFallback() {
        RagExecutionResult actual = RagExecutionResult.error(
                0,
                new IllegalArgumentException("서로 다른 두 매장을 선택해야 합니다.")
        );
        RagExpectedError expected = new RagExpectedError(
                "validation_error", 400, "서로 다른 두 매장을 선택해야 합니다."
        );
        ...
        assertThat(RagEvaluationMetricCalculator.calculateExpectedError(true))
                .containsEntry("expectedErrorMatch", 1.0);
    }
    ...
    void doesNotClassifyDifferentExceptionAsResourceNotFound() {
        ...
        RagExpectedError expected = new RagExpectedError(
                "RESOURCE_NOT_FOUND", 404, "라멘샵을 찾을 수 없습니다."
        );
