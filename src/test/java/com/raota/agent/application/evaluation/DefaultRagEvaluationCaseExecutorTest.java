...
class DefaultRagEvaluationCaseExecutorTest {
    ...
    private final DefaultRagEvaluationCaseExecutor executor = new DefaultRagEvaluationCaseExecutor(searchService,
            reviewSummaryService, followUpChatService, comparisonService, objectMapper);

    @Test
    void executesSearchAndExposesCandidateIdsAndEvidence() {
        when(searchService.search(new AiRamenShopSearchCommand("시오 국물", null))).thenReturn(new AiRamenShopSearchResult(
                List.of(new AiRamenShopSearchResult.ShopResult(7L, "라멘집", "시오", "서울", "담백한 국물", "", 92, false))));

        RagExecutionResult result = executor.execute(new RagEvaluationCase("search-1", RagEvaluationCaseType.SEARCH,
                RagEvaluationSplit.DEV, objectMapper.createObjectNode().put("query", "시오 국물"), "ANONYMOUS",
        ...
        assertThat(result.evidence()).singleElement()
            .satisfies(evidence -> assertThat(evidence.text()).contains("담백한 국물"));
    }

    @Test
    void skipsContractOnlyCaseWithoutCallingProductServices() {
        RagExecutionResult result = executor
            .execute(new RagEvaluationCase("search-contract", RagEvaluationCaseType.SEARCH, RagEvaluationSplit.DEV,
                    objectMapper.createObjectNode().put("query", "거리 가까운 곳"), "AUTHENTICATED", List.of(), false,
                    List.of(), List.of(), List.of(), false, true, 1, 6));
    ...
    void recognizesReviewSummaryFallbackResponse() {
        when(reviewSummaryService
            .summarizeReviewsWithEvidence(org.mockito.ArgumentMatchers.any(ReviewSummaryQuery.class)))
            .thenReturn(new ReviewSummaryService.ReviewSummaryExecution(
                    new ReviewSummaryResponse(
                            new ReviewSummaryResponse.AiShopBasicInfo(7L, "라멘집", "시오", "서울", "", false), 0,
        ...
        RagExecutionResult result = executor.execute(new RagEvaluationCase("summary-1", RagEvaluationCaseType.SUMMARY,
                RagEvaluationSplit.DEV, objectMapper.createObjectNode().put("shopId", 7), "AUTHENTICATED", List.of(),
                false, List.of("리뷰"), List.of(), List.of(), true, false, 1, 6));
