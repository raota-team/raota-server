...
import com.raota.web.community.presentation.request.CommunityCommentCreateRequest;
import com.raota.web.community.presentation.request.CommunityCreatePostRequest;
import com.raota.web.community.infrastructure.persistence.command.JpaCommentRepository;
import com.raota.web.community.infrastructure.persistence.command.JpaPostRepository;
...
import com.raota.web.ramenlog.domain.model.RamenLog;
...
class MemberIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        savedMember = memberRepository.save(MemberProfile.builder().nickname("마이페이지테스터").build());
    ...
    void my_activity_stats_update_realtime() {
        CommunityCreatePostRequest postRequest = new CommunityCreatePostRequest("REVIEW", null, "마이페이지 제목", null,
                "PLAIN", "내용");
        given().header("Authorization", "Bearer " + accessToken)
        ...
        Long realPostId = jdbcTemplate.queryForObject("SELECT id FROM tb_post WHERE title = '마이페이지 제목' LIMIT 1",
                Long.class);
    ...
    void deleted_post_and_its_comments_are_excluded_from_my_page() {
        CommunityCreatePostRequest postRequest = new CommunityCreatePostRequest("REVIEW", null, "삭제될 게시글", null,
                "PLAIN", "내용");
        given().header("Authorization", "Bearer " + accessToken)
            .contentType(ContentType.JSON)
            .body(postRequest)
    ...
    void get_user_public_profile_and_activity_lists_by_user_id() {
        CommunityCreatePostRequest postRequest = new CommunityCreatePostRequest("REVIEW", null, "공개 조회 게시글", null,
                "PLAIN", "내용");
        given().header("Authorization", "Bearer " + accessToken)
            .contentType(ContentType.JSON)
            .body(postRequest)
        ...
        Long postId = jdbcTemplate.queryForObject("SELECT id FROM tb_post WHERE title = '공개 조회 게시글' LIMIT 1",
                Long.class);
    ...
    void private_activity_categories_are_hidden_from_other_users() {
        ...
        given().header("Authorization", "Bearer " + accessToken)
            .contentType(ContentType.JSON)
            .body(Map.of("logs", false, "visits", false, "posts", false, "comments", false))
            .when()
