...
import com.raota.web.account.domain.auth.repository.RefreshTokenRepository;
...
class AuthIntegrationTest extends BaseIntegrationTest {
    ...
    void refresh_token_success() {
        ...
        given().cookie("raota_refresh_token", refreshTokenValue)
            .when()
            .post("/auth/refresh")
            .then()
    ...
    void refresh_token_fail_with_invalid_token() {
        given().cookie("raota_refresh_token", "invalid-token")
            .when()
            .post("/auth/refresh")
            .then()
    ...
    void refresh_token_fail_for_withdrawn_member() {
        MemberProfile member = memberRepository.save(MemberProfile.builder().nickname("withdrawn-user").build());
