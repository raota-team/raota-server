...
import com.raota.web.account.infrastructure.auth.AuthenticatedMember;
...
class AuthenticatedMemberTest {
    ...
    void USER는_일반_사용자_권한만_가진다() {
        ...
        assertThat(member.authorities()).extracting(GrantedAuthority::getAuthority).containsExactly("ROLE_USER");
    }
    ...
    void ADMIN은_일반_사용자와_관리자_권한을_모두_가진다() {
        ...
        assertThat(member.authorities()).extracting(GrantedAuthority::getAuthority)
