...
import com.raota.web.account.domain.auth.model.SocialAccount;
import com.raota.web.account.application.auth.AuthAccountService;
import com.raota.web.account.application.auth.AuthService;
import com.raota.web.account.application.auth.OAuth2LoginResult;
import com.raota.web.account.application.auth.OAuth2UserInfo;
...
import com.raota.web.account.infrastructure.auth.WithdrawnMemberException;
...
public class AuthServiceTest {
    ...
    @BeforeEach
    void setup() {
        AuthProperties properties = new AuthProperties("test-issuer",
                "test-secret-key-at-least-32-characters-long-for-hs256", 3600L, 86400L,
        ...
        info = new OAuth2UserInfo(AuthProvider.GOOGLE, "test", "test@gmail.com", "테스트", "test.jpg");
    ...
    void login_ExistingMemberKeepsMemberEmail() {
        OAuth2UserInfo changedSocialEmailInfo = new OAuth2UserInfo(AuthProvider.GOOGLE, "test", "changed@gmail.com",
                "테스트", "test.jpg");
        ...
        given(authAccountService.findSocialAccount(any())).willReturn(Optional.of(existingAccount));
        given(memberProvisioningService.findById(existingAccount.getMemberId()))
            .willReturn(Optional.of(emailOwnedMemberProfile));
        given(authAccountService.login(any(), any(), eq(emailOwnedMemberProfile.getId())))
    ...
    void login_ExistingSocialAccountButMissingMemberProfile() {
        ...
        given(authAccountService.findSocialAccount(any())).willReturn(Optional.of(existingAccount));
        given(memberProvisioningService.findById(existingAccount.getMemberId())).willReturn(Optional.empty()); // 프로필
                                                                                                               // 없음
        given(memberProvisioningService.createOAuthMember(any(), any())).willReturn(memberProfile); // 새
                                                                                                    // 프로필
                                                                                                    // 생성
        given(authAccountService.login(any(), any(), eq(memberProfile.getId()))).willReturn("test-refresh-token");
    ...
    void login_WithdrawnMember() {
        ...
        assertThatThrownBy(() -> authService.login(info)).isInstanceOf(WithdrawnMemberException.class)
